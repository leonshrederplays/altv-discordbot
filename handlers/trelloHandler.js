...
let conf = {
	...
	serverID: "831913072737517619",
	channelID: "833342594067070976",
	pollInterval: 10000, // 10secs
	contentString: "",
	enabledEvents: [
		"cardCreated",
...
events.on('maxId', (id) => {
    ...
    latestActivityID = id
    fs.writeFileSync('.latestActivityID', parseInt(id).toString())
})
...
const getEmbedBase = (event) => new Discord.MessageEmbed()
        .setFooter(`${conf.guild.members.cache.get(client.user.id).displayName} • ${event.data.board.name} [${event.data.board.shortLink}]`, client.user.displayAvatarURL)
...
const addDiscordUserData = (embed, member) => {
    if (conf.userIDs[member.username]) {
        let discordUser = conf.guild.members.cache.get(conf.userIDs[member.username])
